...
class ImageGenerationService
{
    ...
    private static function generateDeviceImage($deviceId, string $markup, ?Plugin $plugin = null): string
    {
        ...
        $existingImageId = $device->current_screen_image;

        $imageId = self::generateImageFromModel(
            markup: $markup,
        ...
        if ($imageId !== $existingImageId) {
            $device->update(['current_screen_image' => $imageId]);
            Log::info("Device $device->id: updated with new image: $imageId");
        } else {
            Log::info("Device $device->id: image unchanged, keeping: $imageId");
        }
     ...
     * @param  User|null  $user  Optional user for timezone settings
     * @param  DevicePalette|null  $palette  Optional palette, falls back to device model's palette
     ...
     * @param  string|null  $existingImageId  When set, reuse this image id if generated bytes match the stored file
    ...
    public static function generateImageFromModel(
        ...
        try {
            ...
            try {
                ...
                $imageId = self::storeGeneratedImageOrReuseExisting(
                    $localOutputPath,
                    $fileExtension,
                    $existingImageId,
                );
    ...
    /**
     * Store a pipeline output file, reusing an existing image id when bytes match.
     *
     * @return string The stored or reused image id
     *
     * @throws RuntimeException
     */
    private static function storeGeneratedImageOrReuseExisting(
